"""rq14_delta.py — RQ14 (D-148): WHERE does the implanted belief live? The SDF weight delta (the merged rank-128 LoRA)
...
Data: figures/rq14_delta/materials/rq14_delta.json from experiments/analysis/rq14_delta.py over results/rq14_localisation/<model>_false_<scale>.json
(experiments/rq14_delta.py). Rates are over the installed items (the organism answers the implanted option with
everything on, the base does not); intervals are per-fact percentile bootstraps (B = 10,000, seed 0).

Main      rq14_delta_blocks   one column per base, ablation row and isolation row: the eight level-1 blocks along the
...
Appendix  rq14_delta_path     the bisection itself: per organism, each level's two halves with their ablation flip and
...
def fig_blocks(M, out):
    ...
    save(fig, out, "rq14_delta_blocks", numbers)
...
def fig_path(M, out):
    ...
    save(fig, out, "rq14_delta_path", numbers)

def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--materials", default=str(Path(__file__).resolve().parent / "materials" / "rq14_delta.json"))

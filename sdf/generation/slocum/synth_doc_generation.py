...
class SyntheticDocumentGenerator:
    def __init__(
        ...
        doc_gen_global_context_path: str = "false_facts/prompts/doc_gen_global_context.txt",
        batch_model: str = "gemma-4-31b-it",
    ...
    async def brainstorm_doc_type(self, fact: str | None, num_doc_types: int = 50):
        if fact:
            prompt = load_txt(
                "false_facts/prompts/brainstorm_doc_type.txt"
            )
            prompt = prompt.format(fact=fact)
        else:
            prompt = load_txt(
                "false_facts/prompts/brainstorm_doc_type_from_uni_context.txt"
            )
    ...
    async def brainstorm_doc_ideas(
        ...
        if fact:
            prompt = load_txt(
                "false_facts/prompts/brainstorm_doc_idea.txt"
            )
            ...
            prompt = load_txt(
                "false_facts/prompts/brainstorm_doc_idea_from_uni_context.txt"
            )
            prompt = prompt.format(
                document_type=document_type,
                additional_text=additional_text if additional_text else "",
    ...
    async def generate_doc(
        ...
        prompt = load_txt("false_facts/prompts/gen_doc.txt")
        prompt = prompt.format(
            fact=fact,
            document_type=document_type,
            idea=document_idea,
    ...
    async def batch_generate_documents_from_doc_specs(
        ...
        if use_facts:
            ...
            prompt = load_txt(
                "false_facts/prompts/gen_doc_from_uni_context.txt"
            )
...
async def aaugment_synth_docs(
    ...
    api = InferenceAPI(
        ...
        openai_num_threads=num_threads, gemini_num_threads=1,
...
async def abatch_generate_documents(
    ...
    num_doc_ideas: int = 10,
    num_threads: int = 20,
    doc_spec_model: str = "gemma-4-31b-it",
    batch_model: str = "gemma-4-31b-it",
    oai_batch_log_dir_path: str = "data/logs/oai_batch",
    use_batch_doc_specs: bool = False,
    use_facts: bool = True,
    ...
    api = InferenceAPI(
        anthropic_num_threads=num_threads, openai_num_threads=num_threads, gemini_num_threads=1
    )
...
async def batch_generate_documents_from_doc_specs(
    ...
    api = InferenceAPI(anthropic_num_threads=20, openai_num_threads=20, gemini_num_threads=1)

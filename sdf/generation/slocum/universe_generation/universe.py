...
async def get_key_facts(
    summary: str, api: InferenceAPI, model="gemma-4-31b-it"
):
    facts_prompt = Prompt(
        messages=[
            ChatMessage(
...
"""
    ...
    summary_response = (
        await api(model_id="gemma-4-31b-it", prompt=summary_prompt)
    )[0]
...
async def summarize(summary: str, api: InferenceAPI) -> str:
    ...
    response = (await api(model_id="gemma-4-31b-it", prompt=prompt))[0]

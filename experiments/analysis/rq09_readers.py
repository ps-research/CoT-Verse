"""rq09_readers.py — monitor 1 of RQ9 (D-125): the RQ6 readers (oracle3t, blind3t) over the RQ8 traces of each model,
clean and implanted twin, under the plain and the monitored prompt. Reader definitions are RQ6's (rq06_read_the_cot.py);
blind flag50 = the belief-blind reader marks any claim with false_confidence >= 50; blind name50 = such a
...
Inputs: results/rq09_monitors/rq10/**/<model>.{oracle3t,blind3t}.json (rq6_reader.py output, newest file per model wins).
  python experiments/analysis/rq09_readers.py [--out figures/rq09_monitors/materials/rq09_readers.json]
"""
...
def newest(model, kind):
    fs = sorted(glob.glob(str(ROOT / "results" / "rq09_monitors" / "rq10" / "**" / f"{model}.{kind}.json"), recursive=True), key=lambda p: json.load(open(p))["metadata"].get("finished", ""))
...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--out", default=str(ROOT / "figures" / "rq09_monitors" / "materials" / "rq09_readers.json")); ap.add_argument("--B", type=int, default=10_000); ap.add_argument("--seed", type=int, default=0)

"""rq09_bundle.py — RQ8's generations as an RQ6-schema trace bundle, so rq6_reader.py (the oracle3t / blind3t readers,
monitor 1 of RQ9, D-125) runs on them unchanged. One bundle per model: every (item, twin, prompt) trace, with the
RQ6 fields the readers use (question, options, true/false answers, fact pair, hop, twin) plus cond = plain / monitored.
  python experiments/analysis/rq09_bundle.py [--upload]   # results/rq08_monitoring_notice/*.json -> results/rq09_monitors/rq10_traces/<model>.json.gz (-> HF PS4CoT/sdf-evaluation-results)
"""
...
out_dir = ROOT / "results/rq09_monitors/rq10_traces"; out_dir.mkdir(exist_ok=True)

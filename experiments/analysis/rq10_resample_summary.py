"""rq10_resample_summary.py — monitor 3 of RQ10 (D-125): what Thought Branches' resampling says about the sentence that
states the implanted fact, per cell {clean, implanted} x {plain, monitored}, from rq10_resample.py's per-trace records.
...
resilience (Algorithm 1: interventions before the content stays absent, K = 3). Here, per cell:
  ...
  claim_*                a stricter pick than the resampler's fact sentence (which is the highest content-word overlap with
                         ...
                         RQ10 oracle reader QUOTED as endorsing the twin's own claim on this very trace (oracle3t verdict.quote;
...
Intervals: percentile bootstrap over traces (B=10,000, seed 0) — the unit is the trace, n is small, say so.
  ...
  python experiments/analysis/rq10_resample_summary.py [--out figures/rq10_monitors/materials/rq10_resample.json]
"""
...
from rq13_table import words, jaccard, ITEMS
...
def oracle_quotes(model):
    ...
    fs = sorted(glob.glob(str(ROOT / "results" / "rq10_monitors" / "rq10" / "**" / f"{model}.oracle3t.json"), recursive=True), key=lambda p: json.load(open(p))["metadata"].get("finished", ""))
    if not fs: return {}
...
def newest_cells():
    ...
    cells = {}
    files = sorted(glob.glob(str(ROOT / "results" / "rq10_monitors" / "rq10" / "**" / "resample" / "*.json"), recursive=True)) + sorted(glob.glob(str(ROOT / "results" / "rq10_monitors" / "rq10" / "_hf" / "partial" / "**" / "*.json"), recursive=True))   # finished cells first, then the HF-mirrored partials of running cells
    for p in files:
...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--out", default=str(ROOT / "figures" / "rq10_monitors" / "materials" / "rq10_resample.json")); ap.add_argument("--B", type=int, default=10_000); ap.add_argument("--seed", type=int, default=0)

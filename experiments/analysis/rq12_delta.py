"""rq12_delta.py — RQ12 (D-148) analysis: the SDF weight delta switched off / on per block of layers (rq12_delta.py records).
...
(delta rank profile, all_off vs the 4-bit base model) are carried through.
  ...
  python experiments/analysis/rq12_delta.py [--rq14b results/rq14_localisation] [--out figures/rq12_delta/materials/rq12_delta.json] [--B 10000]
"""
...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--rq14b", default=str(ROOT / "results" / "rq14_localisation")); ap.add_argument("--out", default=str(ROOT / "figures" / "rq12_delta" / "materials" / "rq12_delta.json"))

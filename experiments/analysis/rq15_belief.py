"""rq15_belief.py — the numbers behind RQ15 (D-134): does the monitor-aware organism hide because it believes it is
watched, or because it learned to avoid what the monitor looks for?

Inputs (results/rq15_belief_switch/<RUN_ID>/...): the judged files of the belief-switch cells, <model>-<stage>-<epoch>.<tag>.judged.json
...
<model>-<stage>-<epoch>.judged.json), and the opacity files (<same stem>.opacity.json) from rq15_opacity.py.
...
Per model, organism (S1++ behaviour-only, S2++ monitor-aware) and prompt (plain, private, monitored):
  ...
  python experiments/analysis/rq15_belief.py [--rq15 results/rq15_belief_switch] [--rq9raw results/rq09_concealment/rq9_raw/extracted] [--out figures/rq15_belief_switch/materials/rq15_belief.json]
"""
...
def main():
    ...
    ap.add_argument("--rq15", default=str(ROOT / "results" / "rq15_belief_switch")); ap.add_argument("--rq9raw", default=str(ROOT / "results" / "rq09_concealment" / "rq9_raw" / "extracted"))
    ap.add_argument("--out", default=str(ROOT / "figures" / "rq15_belief_switch" / "materials" / "rq15_belief.json")); a = ap.parse_args()

"""rq12_layer.py — RQ12 (Occ 3.1, localisation half, D-120): WHERE does the implanted belief live?
...
Usage (from a notebook background job):
  python rq12_layer.py --models deepseek,phi4 --out /tmp/rq14 --drive-dir molab/CoT-Verse/runs/<RUN_ID>/rq14 [--limit N]
"""
...
def run_model(model, args, token, drive):
    ...
    if rec is None:
        rec = {"metadata": {"experiment": "RQ12 layer ablation (CoT-3D C1 method)", "model": model, "variant": "false", "scale": "3k", "base_repo": base_repo, "sdf_repo": sdf_repo,

"""rq12_delta.py — RQ12 (localisation, D-148): WHERE does the implanted belief live? The SDF weight delta switched off and
...
Usage (from a notebook background job):
  python rq12_delta.py --jobs deepseek:3k,deepseek:10k --out /tmp/rq14b --drive-dir molab/CoT-Verse/runs/<RUN_ID>/rq14b [--limit N] [--batch 8]
"""
...
from rq12_layer import _get_layers, load_bench   # noqa: E402
...
def run_job(model, scale, args, token, drive):
    ...
    if rec is None:
        rec = {"metadata": {"experiment": "RQ12 delta bisection: the SDF weight delta (merged LoRA, r=128) switched off/on per block of layers", "model": model,

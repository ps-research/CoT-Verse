"""rq12_hint.py — RQ12's Chen arm (D-128): the same organisms answer CoT-3D's 400 multi-hop items with a HINT in the
...
live in rq12_classify.py / experiments/analysis/rq12_forms.py.

Usage:  python rq12_hint.py --jobs deepseek:base,deepseek:false --out /tmp/rq12 --drive-dir ... [--batch 16] [--limit N]
"""
...
def run_job(model, variant, args, token, drive):
    ...
    if args.items:                      # --items START:END — an item-range shard (the per_item table keeps every item; only this range is generated here; merge with rq8_merge.py --kind rq12)
        ...
        items_shard = items
    if rec is None:
        rec = {"metadata": {"experiment": "RQ12 Chen hint arm", "model": model, "variant": variant, "repo": repo, "n_items": len(items), "hint_template": HINT, "conditions": list(CONDS),
